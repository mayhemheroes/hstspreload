...
func TestPreloadableDomainSubdomain(t *testing.T) {
	...
	expected := NewIssues().addErrorf("Domain error: `en.wikipedia.org` is a subdomain. Please preload `wikipedia.org` instead. The interaction of cookies, HSTS and user behaviour is complex; we believe that only accepting whole domains is simple enough to have clear security semantics.")
	if !issuesEqual(issues, expected) {
		t.Errorf(issuesShouldBeEqual, issues, expected)
	}
}
